...
class DAOSubstance_Actives_FR extends AbstractDAO {
    ...
    public function get($id) {
        ...
        $req = $this->bdd->prepare('SELECT s.* FROM the (Select classes from Substances_Actives_FR where identifiant = :identifiant) s');
        ...
        if (count($donnee) != 1) {
            return null;
               ...
               foreach($donnee as $data)
                   array_push($classes, new ClasseEntity($data->IDENTIFIANT, $data->LIBELLE,$data->IDPERE)); 
        }
        
        $req = $this->bdd->prepare('SELECT identifiant, libelle FROM Substances_Actives_FR where identifiant = :identifiant');
        $req->execute(array("identifiant" => $id));
        $donnee = $req->FetchAll(PDO::FETCH_OBJ);
        if (count($donnee) != 1) {
            return null;
        } else {
            return new Substance_Actives_FREntity($donnee[0]->IDENTIFIANT, $donnee[0]->LIBELLE,$classes);
        }
    }
    ...
    public function insert($entity) {
        ...
        $req = $this->bdd->prepare('INSERT INTO Substances_Actives_FR (identifiant, libelle, classes) VALUES 
			(:identifiant, :libelle, classe_t(classe(:classeId, :classeLib, :classeIdp)))');
        $req->execute(array(
