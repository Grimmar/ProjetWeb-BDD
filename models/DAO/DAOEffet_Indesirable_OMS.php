...
class DAOEffet_Indesirable_OMS extends AbstractDAO {
    ...
    public function find($a) {
       ...
       $req = $this->bdd->prepare($sqlrequest);
        ...
        $result = array();
        foreach ($req as $data){
            array_push($result, new Effet_Indesirable_OMSEntity($data->IDENTIFIANT, $data->LIBELLE, $data->IDPERE));
        }
        return $result;
    }
    ...
    public function get($id) {
        ...
        if (count($donnee) != 1) {
            ...
            return new Effet_Indesirable_OMSEntity($donnee[0]->IDENTIFIANT, $donnee[0]->LIBELLE, $donnee[0]->IDPERE);
        }
    }
